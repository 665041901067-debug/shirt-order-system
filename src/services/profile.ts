...
export async function saveProfileOnboarding(input: {
  ...
  try {
    ...
    if (userError || !user) {
      return { success: false, error: "ไม่พบข้อมูลผู้ใช้งานที่เข้าสู่ระบบ กรุณาเข้าสู่ระบบใหม่อีกครั้ง" };
    }
    ...
    if (input.new_password && input.new_password.trim().length > 0) {
      ...
      if (pwError) {
        return { success: false, error: translateThaiError(pwError) };
      }
    }
    ...
    if (upsertError) {
      ...
      if (updateError) {
        return { success: false, error: translateThaiError(updateError) };
      }
    }

    return { success: true };
  } catch (err: any) {
    return { success: false, error: translateThaiError(err) };
  }
}
...
export async function changeUserPassword(newPassword: string): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    if (error) {
      return { success: false, error: translateThaiError(error) };
    }
...
export async function checkAndRegisterFirstTimeUser(
  ...
  try {
    ...
    if (signUpErr) {
      return {
        success: false,
        error: translateThaiError(signUpErr) || "รหัสผ่านไม่ถูกต้อง โปรดตรวจสอบรหัสผ่านของคุณ (หรือใช้รหัสนักศึกษาสำหรับเข้าใช้งานครั้งแรก)",
      };
    }
